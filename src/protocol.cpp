...
void all_data_update(void)
{
   ...
   // mcu_dp_raw_update(DPID_MEAL_PLAN, ty_meal_plan,strlen((char*)ty_meal_plan)); //RAW型数据上报;
    mcu_dp_bool_update(DPID_QUICK_FEED,ty_quick_feed); //快速喂食 BOOL型数据上报;
    mcu_dp_value_update(DPID_MANUAL_FEED, ty_manual_feed); //手动喂食 VALUE型数据上报;
    mcu_dp_enum_update(DPID_FEED_STATE, ty_feed_state); //喂食状态 枚举型数据上报;
    mcu_dp_value_update(DPID_BATTERY_PERCENTAGE, ty_battery_percentage); //电池电量 VALUE型数据上报;
    mcu_dp_bool_update(DPID_CHARGE_STATE, ty_charge_state); //充电状态 BOOL型数据上报;
//    mcu_dp_fault_update(DPID_FAULT, ty_fault); //故障型数据上报;
    mcu_dp_value_update(DPID_FEED_REPORT, ty_feed_report); //喂食结果 VALUE型数据上报;
    mcu_dp_value_update(DPID_SURPLUS_GRAIN,ty_surplus_grain); //当前粮桶余粮 VALUE型数据上报;
...
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
extern void  feed_open(uint8_t num);
